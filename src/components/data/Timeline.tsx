...
export interface TimelineItem {
	...
	description: string;
	timestamp: number;
	type: 'bid' | 'claim' | 'stake' | 'donation' | 'transfer' | 'other';
	icon?: LucideIcon;
					...
					>
						...
						>
									...
									<div className="flex items-center space-x-3 mb-2">
										{IconComponent && (
											<div className={cn('p-2 rounded-lg', `${dotColor}/10`)}>
												<IconComponent size={16} className={dotColor.replace('bg-', 'text-')} />
											</div>
										)}
							...
							{item.metadata && Object.keys(item.metadata).length > 0 && (
								<div className="flex flex-wrap gap-2 mt-4 pt-4 border-t border-text-muted/10">
									{Object.entries(item.metadata).map(([key, value]) => (
										...
										>
											{key}: <span className="font-mono text-text-primary">{value}</span>
										</span>
									))}
								</div>
							)}
							...
							{item.expandedContent && (
								<div className="mt-4 pt-4 border-t border-text-muted/10">{item.expandedContent}</div>
							)}
