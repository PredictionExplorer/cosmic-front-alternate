...
apiClient.interceptors.response.use(
	...
	(error: AxiosError) => {
		...
		const customError = {
			message: error.response?.statusText || error.message || 'An unknown error occurred',
...
class CosmicSignatureAPI {
	...
	async getClaimHistoryByUser(address: string) {
		const { data } = await apiClient.get(`prizes/history/by_user/${address}/0/1000000`);
	...
	async getUnclaimedRaffleDeposits(address: string) {
		const { data } = await apiClient.get(`prizes/deposits/unclaimed/by_user/${address}/0/1000000`);
	...
	async getRaffleDepositsByUser(address: string) {
		const { data } = await apiClient.get(`prizes/deposits/raffle/by_user/${address}`);
	...
	async getBidListByRound(roundNum: number, sortDir: 'asc' | 'desc' = 'desc') {
		...
		const { data } = await apiClient.get(`bid/list/by_round/${roundNum}/${dir}/0/1000000`);
	...
	async getCSTTokensByUser(address: string) {
		const { data } = await apiClient.get(`cst/list/by_user/${address}/0/1000000`);
	...
	async searchTokenByName(name: string) {
		const { data } = await apiClient.get(`cst/names/search/${encodeURIComponent(name)}`);
	...
	async getCSTTransfers(address: string) {
		const { data } = await apiClient.get(`cst/transfers/by_user/${address}/0/1000000`);
	...
	async getTokenOwnershipTransfers(tokenId: number) {
		const { data } = await apiClient.get(`cst/transfers/all/${tokenId}/0/1000000`);
	...
	async getCTTransfers(address: string) {
		const { data } = await apiClient.get(`ct/transfers/by_user/${address}/0/1000000`);
	...
	async getETHDonationsWithInfo() {
		const { data } = await apiClient.get('donations/eth/with_info/list/0/1000000');
		return data.DirectCGDonations || [];
	}
	...
	async getETHDonationsByRound(roundNum: number) {
		const { data } = await apiClient.get(`donations/eth/both/by_round/${roundNum}`);
	...
	async getUnclaimedDonatedNFTsByUser(address: string) {
		const { data } = await apiClient.get(`donations/nft/unclaimed/by_user/${address}`);
	...
	async getClaimedDonatedNFTsByUser(address: string) {
		const { data } = await apiClient.get(`donations/nft/claims/by_user/${address}`);
	...
	async getERC20DonationsByRound(roundNum: number) {
		const { data } = await apiClient.get(`donations/erc20/by_round/detailed/${roundNum}`);
	...
	async getStakingCSTActionsByUser(address: string) {
		const { data } = await apiClient.get(`staking/cst/actions/by_user/${address}/0/1000000`);
	...
	async getStakingCSTActions() {
		const { data } = await apiClient.get('staking/cst/actions/global/0/1000000');
		return data.StakingCSTActions || [];
	}
	...
	async getStakedCSTTokensByUser(address: string) {
		const { data } = await apiClient.get(`staking/cst/staked_tokens/by_user/${address}`);
	...
	async getStakingRewardsByUser(address: string) {
		const { data } = await apiClient.get(`staking/cst/rewards/by_user/by_token/summary/${address}`);
	...
	async getStakingCSTRewardsToClaim(address: string) {
		const { data } = await apiClient.get(`staking/cst/rewards/to_claim/by_user/${address}`);
	...
	async getStakingCSTRewardsCollected(address: string) {
		const { data } = await apiClient.get(`staking/cst/rewards/collected/by_user/${address}/0/1000000`);
	...
	async getStakingCSTRewardsByRound(roundNum: number) {
		const { data } = await apiClient.get(`staking/cst/rewards/by_round/${roundNum}`);
		return data.Rewards || [];
	}
	...
	async getStakingRWLKActionsByUser(address: string) {
		const { data } = await apiClient.get(`staking/rwalk/actions/by_user/${address}/0/1000000`);
	...
	async getStakingRWLKActions() {
		const { data } = await apiClient.get('staking/rwalk/actions/global/0/1000000');
	...
	async getStakedRWLKTokensByUser(address: string) {
		const { data } = await apiClient.get(`staking/rwalk/staked_tokens/by_user/${address}`);
	...
	async getStakingRWLKMintsByUser(address: string) {
		const { data } = await apiClient.get(`staking/rwalk/mints/by_user/${address}`);
	...
	async getStakingRWLKMintsGlobal() {
		const { data } = await apiClient.get('staking/rwalk/mints/global/0/1000000');
	...
	async getMarketingRewardsByUser(address: string) {
		const { data } = await apiClient.get(`marketing/rewards/by_user/${address}/0/1000000`);
	...
	async unbanBid(bidId: number) {
		const { data } = await axios.post(`${ASSETS_BASE_URL}unban_bid`, { bid_id: bidId });
		return data;
	}
}
