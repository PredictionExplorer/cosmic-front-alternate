...
export default function MyNFTsPage() {
	...
	const stakedNFTIds = userNFTs.slice(0, user.nftsStaked).map(nft => nft.tokenId);

	const filteredNFTs = userNFTs.filter(nft => {
		// Filter by staking status
		if (filter === 'staked' && !stakedNFTIds.includes(nft.tokenId)) return false;
		if (filter === 'unstaked' && stakedNFTIds.includes(nft.tokenId)) return false;
	...
	return (
		<div className="min-h-screen">
			...
			<section className="section-padding bg-background-surface/50">
				<Container>
					<Breadcrumbs
						items={[{ label: 'My Account', href: '/account' }, { label: 'My NFTs' }]}
						className="mb-8"
					/>
					...
					<motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }}>
						...
						<p className="body-lg">
							You own {user.nftsOwned} Cosmic Signature NFT{user.nftsOwned !== 1 ? 's' : ''} •{' '}
							{user.nftsStaked} currently staked
						</p>
					</motion.div>
				</Container>
			</section>
						...
						<div className="flex items-center space-x-4">
							...
							{selected.length > 0 && (
								<div className="flex items-center space-x-2">
									...
									<Button size="sm" variant="outline" onClick={handleBatchUnstake}>
										Unstake Selected
									</Button>
				...
				<Container>
					{filteredNFTs.length === 0 ? (
						<Card glass className="p-12 text-center">
							<p className="text-text-secondary">No NFTs found matching your criteria.</p>
						</Card>
					) : (
						<>
