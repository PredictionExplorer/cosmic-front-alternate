...
import { Trophy, Gem, Award, TrendingUp, Activity, ArrowRight } from 'lucide-react';
...
export default function AccountDashboardPage() {
	...
	const totalUnclaimedETH =
		parseFloat(formatEth(winnings.unclaimedETH)) +
		winnings.unclaimedERC20Tokens.reduce((sum, token) => sum + 0.1, 0); // Simplified
	...
	return (
		<div className="min-h-screen">
				...
				<Container>
					<motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }}>
						<div className="flex items-center justify-between flex-wrap gap-4 mb-6">
							<div>
								<h1 className="heading-lg mb-2">Welcome back, {user.ensName || 'Collector'}</h1>
							...
							<div className="flex space-x-2">
								{user.isChronoWarrior && (
									<div className="px-4 py-2 rounded-lg bg-status-info/10 border border-status-info/20">
										<span className="text-sm font-medium text-status-info">⚡ Chrono-Warrior</span>
									</div>
								)}
								{user.isEnduranceChampion && (
									<div className="px-4 py-2 rounded-lg bg-primary/10 border border-primary/20">
										<span className="text-sm font-medium text-primary">🏆 Endurance Champion</span>
									</div>
								)}
							</div>
						</div>
			...
			<section className="py-12">
				<Container>
					<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
						...
						<StatCard label="NFTs Staked" value={user.nftsStaked} icon={Award} />
						<StatCard label="Total Bids" value={user.totalBids} icon={Activity} />
						<StatCard
							label="Total Won"
					...
					<div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
						...
						<Card glass className="p-8">
							...
							<div className="space-y-6">
								...
								<div>
									<div className="flex justify-between items-baseline mb-2">
										<span className="text-sm text-text-secondary">Total Won</span>
						...
						{/* Prize Breakdown */}
						<Card glass className="p-8">
							<h3 className="font-serif text-2xl font-semibold text-text-primary mb-6">Prize Wins</h3>
			...
			<section className="py-12">
				<Container>
					<Card glass className="p-8">
						<h3 className="font-serif text-2xl font-semibold text-text-primary mb-6">Quick Actions</h3>
			...
			<section className="py-12 bg-background-surface/50">
				<Container>
					<div className="flex items-center justify-between mb-8">
						<h2 className="font-serif text-3xl font-semibold text-text-primary">Recent Activity</h2>
					...
					<Timeline
						items={recentActivities.map(activity => ({
							...
							link: activity.type === 'bid' ? `/game/history/bids/${activity.id}` : undefined
						}))}
					/>
				</Container>
			</section>
			...
			<section className="py-12">
				<Container>
					<div className="flex items-center justify-between mb-8">
						<h2 className="font-serif text-3xl font-semibold text-text-primary">Your Collection</h2>
						<Button variant="ghost" asChild>
							<Link href="/account/nfts">
								View All {user.nftsOwned} NFTs
								<ArrowRight className="ml-2" size={16} />
							</Link>
						</Button>
					</div>
				...
				<Container>
					<Card glass className="p-8">
						<div className="flex items-center justify-between">
							...
							<div className="text-right">
								<div className="font-mono text-4xl font-bold text-primary mb-2">
									{formatCst(parseFloat(user.totalCSTWon) - parseFloat(user.totalCSTSpent))}
								</div>
						...
						<div className="mt-6 pt-6 border-t border-text-muted/10 grid grid-cols-2 gap-6">
							<div>
								<p className="text-xs text-text-secondary mb-1 uppercase tracking-wide">Earned</p>
								<p className="font-mono text-lg text-status-success">
									{formatCst(user.totalCSTWon)} CST
								</p>
							</div>
							<div>
								<p className="text-xs text-text-secondary mb-1 uppercase tracking-wide">Spent</p>
