...
import { Trophy, Coins, Gem, ArrowRight, CheckCircle, Package } from 'lucide-react';
	...
	if (!hasUnclaimedPrizes && claimedHistory.length === 0) {
		return (
			<div className="min-h-screen section-padding">
				<Container>
					<Breadcrumbs
						items={[{ label: 'My Account', href: '/account' }, { label: 'Winnings' }]}
						className="mb-8"
					/>
		...
		<div className="min-h-screen">
			...
			{hasUnclaimedPrizes && (
				<section className="py-12">
					<Container>
						<h2 className="font-serif text-3xl font-semibold text-text-primary mb-8">Ready to Claim</h2>
										...
										<div className="flex items-start space-x-4">
											...
											<div>
												...
												<p className="text-sm text-text-muted">
													≈ $
													{(parseFloat(formatEth(winnings.unclaimedETH)) * 2400).toFixed(2)}{' '}
													USD
												</p>
											</div>
										</div>
														...
														>
															<div>
																<span className="font-mono text-lg text-text-primary">
																	{(parseFloat(token.amount) / 1e6).toLocaleString()}{' '}
																	{token.tokenSymbol}
																</span>
															</div>
											...
											<div className="flex items-start space-x-4 mb-6">
												...
												<div>
													...
													<p className="text-text-secondary">
														...
														{winnings.unclaimedDonatedNFTs.length > 1 ? 's' : ''} donated by
														other players
													</p>
												</div>
											</div>
			...
			<section className="py-12">
				<Container>
					...
					>
						<h2 className="font-serif text-3xl font-semibold text-text-primary">Claimed History</h2>
