...
export default function GalleryPage() {
	...
	useEffect(() => {
		async function fetchNFTs() {
			try {
				...
				const transformed = nfts.map((nft: any) => ({
	...
	const filteredNFTs = allNFTs
		...
		.sort((a, b) => {
			if (sortBy === 'newest') return new Date(b.mintedAt).getTime() - new Date(a.mintedAt).getTime();
			if (sortBy === 'oldest') return new Date(a.mintedAt).getTime() - new Date(b.mintedAt).getTime();
				...
				<Container>
					...
					>
						...
						<p className="body-xl max-w-3xl mx-auto">
							Explore the complete collection of Cosmic Signature NFTs. Each piece is a unique work of
							generative art, created through a deterministic algorithm with a verifiable seed.
						</p>
					</motion.div>
				</Container>
			</section>
				...
				<Container>
					<div className="py-6 flex flex-col md:flex-row items-center justify-between gap-4">
						...
						<div className="flex items-center space-x-4">
							...
							<select
								value={sortBy}
								onChange={e => setSortBy(e.target.value as 'newest' | 'oldest' | 'id')}
							...
							{viewMode === 'grid' ? (
								<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
									{filteredNFTs.map((nft, index) => (
										<NFTCard key={nft.id} nft={nft} delay={Math.min(index * 0.05, 0.4)} />
									))}
								</div>
							) : (
											...
											<Card glass hover className="p-6">
												<div className="flex items-center space-x-6">
													...
													<div className="flex-1 min-w-0">
														...
														<p className="text-xs text-text-muted">Owner: {nft.owner}</p>
													</div>
