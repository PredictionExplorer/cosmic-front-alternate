...
import { Trophy, Timer, Sparkles, ArrowRight, TrendingUp, Award, Gem, Users } from 'lucide-react';
...
export default function Home() {
	...
	const { useRoundNum, useLastBidder, useMainPrizeTime, useCurrentChampions } = useCosmicGameRead();
	const [featuredNFTs, setFeaturedNFTs] = useState<Array<Record<string, unknown>>>([]);
	...
	const currentRound = {
		roundNumber: roundNum?.toString() || dashboardData?.CurRoundNum?.toString() || '0',
		prizePool: (dashboardData?.PrizeAmountEth as number) || 0,
		totalBids: (dashboardData?.CurNumBids as number) || 0,
		lastBidder: lastBidder ? shortenAddress(lastBidder as string, 6) : 'Waiting...',
		totalNFTs: ((dashboardData?.MainStats as Record<string, unknown>)?.NumCSTokenMints as number) || 0,
		contractBalance: (dashboardData?.CosmicGameBalanceEth as number) || 0
	};

	// Extract champion addresses safely
	const championsArray = championsData as unknown as [string, bigint, string, bigint] | undefined;
	const enduranceChampion = championsArray?.[0] ? shortenAddress(championsArray[0], 6) : 'TBD';
	const chronoWarrior = championsArray?.[2] ? shortenAddress(championsArray[2], 6) : 'TBD';
	...
	useEffect(() => {
		async function fetchPrice() {
			try {
				...
				setEthBidPrice(priceData?.ETHPrice ? parseFloat(priceData.ETHPrice) / 1e18 : 0);
			} catch (error) {
		...
		<div className="overflow-hidden">
			...
			<section className="relative min-h-[90vh] flex items-center justify-center">
					...
					<div className="text-center space-y-8">
						...
						>
							<h1 className="heading-xl text-balance mb-6">
								...
								<span className="block text-gradient mt-2">Exceptional Art</span>
							</h1>
							<p className="body-xl max-w-3xl mx-auto text-balance">
								Experience the intersection of sophisticated blockchain gaming and premium NFT
								collecting. Compete in strategic auctions to win substantial prizes and unique digital
								art.
							</p>
						</motion.div>
				...
				>
					<div className="flex flex-col items-center space-y-2">
						<span className="text-xs text-text-secondary uppercase tracking-wider">Scroll to explore</span>
						<motion.div
							animate={{ y: [0, 8, 0] }}
			...
			<section className="section-padding bg-background-surface/50">
				<Container>
					...
					>
						<Card glass className="p-8 md:p-12 text-center" style={{ minHeight: '400px' }}>
							<div className="mb-8">
							...
							<div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8">
								<div>
									...
									<div className="text-sm text-text-secondary">ETH Prize Pool</div>
								</div>
								<div>
									<div className="font-mono text-4xl font-semibold text-primary mb-2">
										{currentRound.totalBids}
							...
							<div className="mt-8">
								<div className="mb-4 text-sm text-text-secondary">
									...
									<span className="text-primary font-mono">{ethBidPrice.toFixed(6)} ETH</span>
								</div>
					...
					<div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-6">
						<StatCard label="Endurance Champion" value={enduranceChampion} icon={Award} delay={0.3} />
						<StatCard label="Chrono-Warrior" value={chronoWarrior} icon={Timer} delay={0.4} />
					</div>
				</Container>
			</section>
						...
						{[
							{
								...
								features: ['Unique artwork', 'Verifiable rarity', 'Stakeable assets']
							},
							{
								icon: Sparkles,
								title: 'Compete',
								...
								features: ['Main Prize: 25% ETH', 'Champion Prizes', 'Raffle Rewards']
							},
							{
								icon: TrendingUp,
								title: 'Earn',
								...
								features: ['6% to stakers', '100 CST per bid', 'Compound gains']
							}
						].map((pillar, index) => (
							<motion.div
								key={pillar.title}
			...
			<section className="section-padding bg-background-surface/50">
				<Container>
					<motion.div
						...
						className="text-center mb-16"
					>
						<h2 className="heading-md text-balance mb-4">Featured Collection</h2>
						<p className="body-lg max-w-2xl mx-auto mb-8">
							Each NFT is a unique piece of generative art, created from verifiable on-chain seeds
						</p>
					</motion.div>
						...
						{isLoadingNFTs ? (
							...
							<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
								{featuredNFTs.map((nft, index) => (
									<NFTCard key={nft.id} nft={nft} delay={index * 0.1} priority={index < 3} />
								))}
							</div>
						) : (
			...
			<section className="section-padding">
				<Container>
					<motion.div
						...
						className="text-center mb-16"
					>
						<h2 className="heading-md text-balance mb-4">How Sophisticated Players Win</h2>
						<p className="body-lg max-w-2xl mx-auto">
						...
						<div className="space-y-6">
							<Card glass className="p-8">
								...
								<div className="space-y-4">
									{[
										...
										{ label: 'Chrono-Warrior', value: '8%', color: 'bg-status-info' },
										{ label: 'Staking Rewards', value: '6%', color: 'bg-status-success' },
										{ label: 'Charity', value: '7%', color: 'bg-status-error' },
										{ label: 'Raffle Prizes', value: '4%', color: 'bg-status-warning' }
									].map(item => (
							...
							<div className="grid grid-cols-2 gap-4">
								...
								<Card glass className="p-6">
									...
									<div className="font-mono text-2xl font-semibold text-primary mb-1">15+</div>
									<div className="text-sm text-text-secondary">Prize Winners</div>
								</Card>
							</div>
						</div>
					</div>
				...
				<Container className="relative z-10">
					...
					>
						<Card glass className="p-12 md:p-16">
							<h2 className="heading-lg text-balance mb-6">Ready to Compete?</h2>
							<p className="body-xl max-w-3xl mx-auto mb-10">
								Join the most sophisticated NFT auction game on the blockchain. Every bid earns rewards.
								Every round creates opportunities.
							</p>
							...
							<div className="mt-12 grid grid-cols-1 md:grid-cols-3 gap-6 max-w-3xl mx-auto">
								<div className="text-center">
									...
									<div className="text-sm text-text-secondary">Audited Smart Contracts</div>
								</div>
								<div className="text-center">
									<div className="text-2xl mb-2">⚡</div>
									<div className="text-sm text-text-secondary">Built on Arbitrum</div>
								</div>
								<div className="text-center">
									<div className="text-2xl mb-2">🎨</div>
									<div className="text-sm text-text-secondary">Unique Generative Art</div>
								</div>
							</div>
						</Card>
					</motion.div>
