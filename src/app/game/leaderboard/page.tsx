...
export default function LeaderboardPage() {
	...
	const [category, setCategory] = useState<'prizes' | 'bids' | 'spending'>('prizes');
		...
		async function fetchLeaderboard() {
			try {
				...
				if (category === 'prizes') {
					...
					data = winners.slice(0, 50).map((winner: any, index: number) => ({
						rank: index + 1,
						address: winner.Address || '0x0',
						value: winner.TotalWinningsEth || 0,
						nftsWon: winner.NumWinnings || 0
					}));
				} else if (category === 'bids') {
					// Get top bidders
					const bidders = await api.getUniqueBidders();
					data = bidders.slice(0, 50).map((bidder: any, index: number) => ({
						rank: index + 1,
						address: bidder.Address || '0x0',
						value: bidder.NumBids || 0,
						nftsWon: bidder.NumWinnings || 0
					}));
				} else {
					// Get top spenders
					const bidders = await api.getUniqueBidders();
					...
					const sorted = [...bidders].sort(
						(a: any, b: any) => (b.TotalSpentEth || 0) - (a.TotalSpentEth || 0)
					);
					data = sorted.slice(0, 50).map((bidder: any, index: number) => ({
						rank: index + 1,
						address: bidder.Address || '0x0',
						value: bidder.TotalSpentEth || 0,
						nftsWon: bidder.NumWinnings || 0
					}));
				}
			...
			<section className="section-padding bg-background-surface/50">
				<Container>
					...
					>
						...
						<p className="body-xl">
							Discover the top players, biggest winners, and most active participants in Cosmic Signature
						</p>
					</motion.div>
				</Container>
			</section>
				...
				<Container>
					{!isLoading && leaderboardData.length >= 3 && (
						<div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
							...
							>
								<Card glass className="p-6 text-center border-accent-platinum/20">
									<Medal size={32} className="text-accent-platinum mx-auto mb-3" />
									<div className="font-mono text-sm text-text-secondary mb-2">#2</div>
									<h3 className="font-serif text-xl font-semibold text-text-primary mb-1">
										{leaderboardData[1]?.ensName || shortenAddress(leaderboardData[1]?.address)}
									</h3>
							...
							>
								<Card glass className="p-8 text-center border-primary/30 shadow-luxury-lg">
									<Trophy size={40} className="text-primary mx-auto mb-4" />
									<div className="font-mono text-sm text-text-secondary mb-2">#1</div>
									<h3 className="font-serif text-2xl font-semibold text-text-primary mb-2">
										{leaderboardData[0]?.ensName || shortenAddress(leaderboardData[0]?.address)}
									</h3>
							...
							>
								<Card glass className="p-6 text-center border-status-warning/20">
									<Medal size={32} className="text-status-warning mx-auto mb-3" />
									<div className="font-mono text-sm text-text-secondary mb-2">#3</div>
									<h3 className="font-serif text-xl font-semibold text-text-primary mb-1">
										{leaderboardData[2]?.ensName || shortenAddress(leaderboardData[2]?.address)}
									</h3>
													...
													>
														...
														<td className="p-4">
															<p className="font-mono text-text-primary">
																{entry.ensName || shortenAddress(entry.address, 6)}
															</p>
														</td>
														<td className="p-4 text-right">
