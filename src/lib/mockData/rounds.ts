...
export const MOCK_ROUNDS: RoundData[] = Array.from({ length: 234 }, (_, i) => {
	...
	const daysAgo = i * 3; // Rounds every ~3 days
	...
	const charityAmount = deterministicValue(roundNum + 4000, 0.7, 1.0).toFixed(4);

	const bids = Math.floor(deterministicValue(roundNum + 5000, 35, 65));

	// Generate deterministic addresses
	const winnerHash = ((roundNum * 7919 + 6571) % 999999).toString(16).padStart(8, '0');
	const enduranceHash = ((roundNum * 8831 + 7753) % 999999).toString(16).padStart(8, '0');
	const chronoHash = ((roundNum * 9743 + 8837) % 999999).toString(16).padStart(8, '0');

	return {
		roundNum,
		winner: `0x${winnerHash}...`,
		winnerENS: roundNum === 234 ? 'alice.eth' : roundNum === 233 ? 'bob.eth' : undefined,
		...
		duration: Math.floor(deterministicValue(roundNum + 7000, 24 * 3600, 72 * 3600)), // 1-3 days
		ethCollected: (
